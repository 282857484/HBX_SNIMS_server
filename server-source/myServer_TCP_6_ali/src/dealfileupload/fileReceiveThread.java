...
public class fileReceiveThread implements Runnable {
	...
	public void run() {
		...
		if (client.isInputShutdown()) {
			System.out
					.println("fileReceiveThread-after client import : inputShutdown");
		}
		if (client.isOutputShutdown()) {
			System.out
					.println("fileReceiveThread-after client import : outputShutdown");
		}
		try {
			...
			byte[] sendBytes = new byte[FileConfig.CacheSize];
			int transLen = 0;
...
//					break;
			...
			System.out.println("----接收文件<" + fileName + ">成功----" + "接收大小: "
					+ transLen + "byte");
			if (client != null)
				client.close();
			if (dis != null)
			...
			if (!Server.checkFileProcessingExist(Path)) {
				...
				PhotoCompressTool.CompressPhoto(inputFilePath, outputFilePath,
						inputFileName, outputFileName, FileConfig.XWidth,
						FileConfig.XHeight, FileConfig.IsZoomInScale);
			...
			if(ENDCODEMARK == false) {
				H_Founction.TraceError(BugLevel.BUG, "", "", "", "File receive false");
			}
		...
		// System.out.println("Compress ELEMENTS : " + inputFilePath + " , " +
		// outputFilePath + " , " + inputFileName + " , " + outputFileName);
